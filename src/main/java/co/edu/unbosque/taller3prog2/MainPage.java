...
public class MainPage extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(Objects.equals(rol, "propietario")) {
            response.sendRedirect(request.getContextPath()+"/propietario.html");
        } else {
            response.sendRedirect(request.getContextPath()+"/funcionario.jsp");
        }

        RWJson.writeToJson(email, password, rol);
        out.println(RWJson.readJson());
    }
